            ...
            self.transformer = nn.ModuleDict(
                {
                    "h": nn.ModuleList(
                        [
                            nn.ModuleDict({"self_attn": SimpleLoRALayer(768, 768), "mlp": SimpleLoRALayer(768, 768)})
                            for _ in range(2)  # Just 2 layers for speed
                        ]
                    )
                }
            )
            self.config = type("Config", (), {"hidden_size": 768, "num_hidden_layers": 2, "model_type": "gpt2"})()
