...
class TestExtensibleCollectorIntegration:
    ...
    def test_full_training_integration(self, temp_dir, mock_model, mock_optimizer):
        ...
        for step in [50, 100, 150, 200]:
            ...
            loss = torch.sum(
                torch.stack([torch.sum(layer.lora_A["default"].weight) for layer in mock_model.base_model.layers])
            )
            loss.backward()

            # Collect metrics
            metrics = metrics_collector.collect_step_metrics(
                step=step, logs={"train_loss": float(loss), "learning_rate": 1e-4}
            )
    ...
    def test_runtime_collector_management(self, mock_model, mock_optimizer):
        ...
        metrics_collector = MetricsCollector(enabled_collectors={DataType.ADAPTER_WEIGHTS})
        metrics_collector.setup(mock_model, mock_optimizer)

        # Collect initial metrics
        metrics1 = metrics_collector.collect_step_metrics(step=100, logs={"train_loss": 2.5})

        # Should not have parameter norms yet
        assert "total_param_norm" not in metrics1

        # Add custom collector at runtime
        metrics_collector.add_collector(DataType.PARAMETER_NORMS, CustomTestCollector)

        # Collect metrics again
        metrics2 = metrics_collector.collect_step_metrics(step=150, logs={"train_loss": 2.3})
        ...
        # Collect metrics again
        metrics3 = metrics_collector.collect_step_metrics(step=200, logs={"train_loss": 2.1})
