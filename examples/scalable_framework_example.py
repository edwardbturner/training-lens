...
from training_lens.core import TrainingLensFramework, DataType
from training_lens.core.integration import create_lora_focused_framework, create_full_spectrum_framework
from training_lens.core.registry import CollectorRegistry, AnalyzerRegistry
from training_lens.core.base import DataCollector, DataAnalyzer

# Import existing components
from training_lens.collectors import AdapterWeightsCollector, AdapterGradientsCollector
from training_lens.analyzers import LoRAAnalyzer, ConvergenceAnalyzer
...
def main():
    ...
    class DummyLoRAModel(torch.nn.Module):
        def __init__(self):
            ...
            self.linear.lora_A = {"default": torch.nn.Linear(10, 2, bias=False)}
            self.linear.lora_B = {"default": torch.nn.Linear(2, 5, bias=False)}
    ...
    analysis_results = framework.analyze_training_data(
        output_dir=Path("./framework_analysis_output")
    )
    ...
    print(f"   Registered new collector: AttentionPatternsCollector")
    ...
    print(f"   Registered new analyzer: AttentionAnalyzer")
    ...
    config = framework.create_pipeline_config(
        save_path=Path("./framework_config.json")
    )
    ...
    print(f"   Created custom framework with specialized configuration")
    
    # === 8. Export and Data Management ===
    print("\n💾 8. Data Export and Management")
    
    # Export collected data
    exported_files = framework.export_data(
        output_dir=Path("./exported_training_data"),
        format="json"
    )
