...
class LoRAAnalyzer(DataAnalyzer):
    ...
    def _analyze_single_adapter_module(
        ...
        for step in steps:
            ...
            if (isinstance(step_data, dict) and 
                'adapter_weights' in step_data and 
                module_name in step_data['adapter_weights']):
    ...
    def _perform_cross_analysis(self, data: Dict[DataType, Any]) -> Dict[str, Any]:
        ...
        if (DataType.ADAPTER_WEIGHTS in data and 
            DataType.ADAPTER_GRADIENTS in data):
        ...
        if (DataType.ADAPTER_WEIGHTS in data and 
            DataType.LORA_ACTIVATIONS in data):
