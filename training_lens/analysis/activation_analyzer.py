    ...
    def register_lora_activation_points(self, adapter_name: str = "default") -> None:
        ...
        try:
            ...
            for module_name, module in lora_modules.items():
                ...
                self.register_activation_point(
                    f"lora_{module_name}_pre_A", 
                    module_name
                )
                ...
                if hasattr(module, 'lora_A') and hasattr(module, 'lora_B'):
                    ...
                    a_handle = module.lora_A[adapter_name].register_forward_hook(
                        make_hook_fn(f"lora_{module_name}")
                    )
            ...
            for act_name in reference_activations.keys():
                if act_name in step_activations:
                    ...
                    if similarity_metric == "cosine":
                        ...
                        similarity = -torch.nn.functional.kl_div(
                            step_prob.log(), ref_prob, reduction='sum'
                        ).item()
                    else:
    ...
    def export_activations(
        ...
        for step, activations in self.activation_data.items():
            if format == "npz":
                ...
                numpy_activations = {
                    name: tensor.numpy() for name, tensor in activations.items()
                }
                ...
                json_activations = {
                    name: tensor.tolist() for name, tensor in activations.items()
                }
    ...
    def _analyze_evolution_patterns(self, all_activations: Dict[int, Dict[str, torch.Tensor]]) -> Dict[str, Any]:
        ...
        for act_name in activation_names:
            ...
            for i in range(1, len(activations_over_time)):
                ...
                similarity = torch.nn.functional.cosine_similarity(
                    prev_act.unsqueeze(0), curr_act.unsqueeze(0)
                ).item()
            ...
            analysis["evolution_patterns"][act_name] = {
                ...
                "stability": "high" if np.mean(similarities) > 0.9 else "medium" if np.mean(similarities) > 0.7 else "low"
            }
    ...
    def _upload_activations_to_hf(self, output_dir: Path) -> None:
        ...
        for file_path in output_dir.glob("*"):
            if file_path.is_file():
                ...
                self.hf_integration.upload_file(
                    file_path,
                    relative_path,
                    f"Upload activation data: {file_path.name}"
                )