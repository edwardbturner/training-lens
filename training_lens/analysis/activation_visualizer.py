...
class ActivationVisualizer:
    ...
    def plot_activation_distribution_evolution(
        ...
        if interactive and PLOTLY_AVAILABLE:
            return self._create_interactive_distribution_plot(
                valid_steps, distributions, activation_name, save_path
            )
        else:
            return self._create_static_distribution_plot(
                valid_steps, distributions, activation_name, save_path
            )
    ...
    def _create_interactive_lora_plot(
        ...
        fig.update_layout(
            title='LoRA vs Main Path Contribution Evolution',
            height=400 * ((n_modules + 2) // 3),
            showlegend=True
        )
    ...
    def _plot_magnitude_evolution_subplot(
        self,
        activation_data: Dict[int, Dict[str, np.ndarray]],
        ax: Axes
    ) -> None:
    ...
    def _plot_variance_evolution_subplot(
        self,
        activation_data: Dict[int, Dict[str, np.ndarray]],
        ax: Axes
    ) -> None:
    ...
    def _plot_similarity_heatmap_subplot(
        self,
        similarity_data: Dict[int, Dict[str, float]],
        ax: Axes
    ) -> None:
    ...
    def _plot_lora_contribution_subplot(
        self,
        lora_data: Dict[int, Dict[str, Dict[str, float]]],
        ax: Axes
    ) -> None:
