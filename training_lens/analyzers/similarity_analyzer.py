...
class SimilarityAnalyzer(DataAnalyzer):
    ...
    def _compute_temporal_similarities(
        ...
        for i in range(1, len(steps)):
            ...
            if i > 0:
                ...
                consecutive_sim = self._compute_data_similarity(
                    data_type, prev_data, current_data
                )
                if consecutive_sim is not None:
                    consecutive_similarities.append(consecutive_sim)
            
            # Reference similarity (with first step)
            ref_sim = self._compute_data_similarity(
                data_type, reference_data, current_data
            )
        ...
        return {
            ...
            "similarity_trend": self._compute_trend(np.array(consecutive_similarities)) if consecutive_similarities else "stable",
            "stability_score": 1 - np.std(consecutive_similarities) if consecutive_similarities else 1,
        }
    
    def _compute_data_similarity(
        self, 
        data_type: DataType, 
        data1: Any, 
        data2: Any
    ...
    def _compute_weight_similarity(self, weight_data1: Any, weight_data2: Any) -> Optional[float]:
        ...
        if (not isinstance(weight_data1, dict) or 'adapter_weights' not in weight_data1 or
            not isinstance(weight_data2, dict) or 'adapter_weights' not in weight_data2):
            return None
    ...
    def _compute_gradient_similarity(self, grad_data1: Any, grad_data2: Any) -> Optional[float]:
        ...
        if (not isinstance(grad_data1, dict) or 'adapter_gradients' not in grad_data1 or
            not isinstance(grad_data2, dict) or 'adapter_gradients' not in grad_data2):
            return None
    ...
    def _compute_activation_similarity(self, act_data1: Any, act_data2: Any) -> Optional[float]:
        ...
        for point_name in common_points:
            ...
            if (isinstance(point1, dict) and isinstance(point2, dict) and
                'statistics' in point1 and 'statistics' in point2):
        ...
        for step in sorted(common_steps):
            ...
            for i, type1 in enumerate(available_types):
                for type2 in available_types[i+1:]:
                    ...
                    similarity = self._compute_cross_type_similarity(
                        type1, data[type1][step],
                        type2, data[type2][step]
                    )
    ...
    def _compute_reference_similarities(
        ...
        return {
            "to_initial": {
                ...
                "trend": self._compute_trend(np.array(similarities_to_initial)) if similarities_to_initial else "stable",
            },
            "to_final": {
