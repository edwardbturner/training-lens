...
class ConvergenceAnalyzer(DataAnalyzer):
    ...
    def _extract_weight_convergence_signals(self, weight_data: Dict[int, Any], steps: List[int]) -> Dict[str, Any]:
        ...
        recent_stability = np.std(recent_norms)
        
        # Compute rate of change
        changes = np.diff(total_norms)
        recent_changes = changes[-recent_window:] if len(changes) >= recent_window else changes
        avg_change_rate = np.mean(np.abs(recent_changes)) if recent_changes.size > 0 else 0
    ...
    def _extract_activation_convergence_signals(self, activation_data: Dict[int, Any], steps: List[int]) -> Dict[str, Any]:
        ...
        recent_stability = np.std(recent_magnitudes)
    ...
    def _analyze_cross_type_convergence(self, data: Dict[DataType, Any], available_types: List[DataType]) -> Dict[str, Any]:
