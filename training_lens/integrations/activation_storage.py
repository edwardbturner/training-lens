...
class ActivationStorage:
    ...
    def list_stored_data(self, model_name: Optional[str] = None, include_remote: bool = True) -> List[Dict[str, Any]]:
    ...
    def create_activation_dataset(
        self, data_ids: List[str], dataset_name: str, description: Optional[str] = None, upload_to_hub: bool = True
    ) -> str:
    ...
    def compute_activation_statistics(
        self, data_ids: Optional[List[str]] = None, model_name: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def cleanup_old_data(
        self, max_age_days: int = 30, keep_latest_n: int = 10, model_name: Optional[str] = None
    ) -> int:
    ...
    def _setup_hf_repo(self, create_if_not_exists: bool) -> None:
        ...
        if self.repo_id is None:
            raise ValueError("repo_id is required for HuggingFace repository setup")
    ...
    def _load_metadata(self) -> Dict[str, Any]:
        ...
        return {"storage_version": "1.0", "created_timestamp": time.time(), "stored_data": {}, "datasets": {}}
    ...
    def _find_data_id(self, checkpoint_step: Optional[int], model_name: Optional[str]) -> Optional[str]:
    ...
    def _upload_to_hub(self, data_dir: Path, data_id: str) -> None:
        ...
        if self.repo_id is None:
            raise ValueError("repo_id is required for uploading to HuggingFace Hub")

        for file_path in data_dir.glob("*"):
            if file_path.is_file():
                repo_path = f"activation_data/{data_id}/{file_path.name}"
                self.hf_api.upload_file(
                    path_or_fileobj=str(file_path), path_in_repo=repo_path, repo_id=self.repo_id, token=self.token
                )

    def _download_from_hub(self, data_id: str) -> None:
        """Download data from HuggingFace Hub."""
        if self.repo_id is None:
            raise ValueError("repo_id is required for downloading from HuggingFace Hub")
        ...
        try:
            repo_files = self.hf_api.list_repo_files(repo_id=self.repo_id, token=self.token)
    ...
    def _list_remote_data(self, model_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if self.repo_id is None:
            raise ValueError("repo_id is required for listing remote data")
        ...
        try:
            ...
            for metadata_file in metadata_files:
                try:
                    ...
                    temp_file = hf_hub_download(repo_id=self.repo_id, filename=metadata_file, token=self.token)
    ...
    def _upload_dataset_to_hub(self, dataset_dir: Path, dataset_id: str) -> None:
        ...
        if self.repo_id is None:
            raise ValueError("repo_id is required for uploading dataset to HuggingFace Hub")
