...
from ..analysis.activation.cli_analyzer import CLIActivationAnalyzer
from ..analysis.activation.visualizer import ActivationVisualizer
...
@click.option(
    "--input-text", "-t", type=str, default="Hello world, this is a test.", help="Input text to analyze activations for"
)
@click.option("--input-file", "-f", type=click.Path(exists=True), help="File containing input texts (one per line)")
...
@click.option(
    "--similarity-metric",
    type=click.Choice(["cosine", "l2", "kl_div"]),
    default="cosine",
    help="Similarity metric for comparing activations",
)
@click.option("--reference-step", type=int, help="Reference checkpoint step for similarity analysis")
@click.option(
    "--export-format", type=click.Choice(["npz", "pt", "json"]), default="npz", help="Export format for activation data"
)
...
def analyze(
    ...
    # Initialize analyzer
    analyzer = CLIActivationAnalyzer(checkpoint_dir=checkpoint_dir, model_name=model_name, hf_repo_id=hf_repo)
    ...
    if analyzer.activation_data:
        ...
        similarity_results = analyzer.compute_activation_similarities(
            reference_step=reference_step, similarity_metric=similarity_metric
        )
    ...
    exported_files = analyzer.export_activations(
        output_dir / "activation_data", format=export_format, upload_to_hf=upload_to_hf
    )
    ...
    if visualize:
        ...
        # Evolution plot
        if viz_data:
            visualizer.plot_activation_evolution(
                viz_data,
            ...
            if "similarity_analysis" in analysis_results:
                ...
                visualizer.plot_activation_similarity_heatmap(
                    similarity_data,
                    ref_step,
            ...
            # Dashboard
            visualizer.create_activation_summary_dashboard(
                viz_data,
    ...
    if hf_repo:
        ...
        storage = ActivationStorage(local_dir=output_dir / "storage", repo_id=hf_repo, create_repo_if_not_exists=True)
...
def list_stored(
    storage_dir: str, model_name: Optional[str], hf_repo: Optional[str], include_remote: bool, output_format: str
) -> None:
    """List stored activation data."""

    storage = ActivationStorage(local_dir=storage_dir, repo_id=hf_repo)

    stored_data = storage.list_stored_data(model_name=model_name, include_remote=include_remote)
...
def create_dataset(
    ...
    try:
        dataset_id = storage.create_activation_dataset(
            data_ids=list(data_ids), dataset_name=dataset_name, description=description, upload_to_hub=upload_to_hf
        )
...
def statistics(storage_dir: str, model_name: Optional[str], data_ids: Optional[str], output: Optional[str]) -> None:
    ...
    try:
        stats = storage.compute_activation_statistics(data_ids=data_id_list, model_name=model_name)
...
def cleanup(storage_dir: str, max_age_days: int, keep_latest: int, model_name: Optional[str], dry_run: bool) -> None:
    ...
    if dry_run:
        ...
        try:
            cleaned_count = storage.cleanup_old_data(
                max_age_days=max_age_days, keep_latest_n=keep_latest, model_name=model_name
            )
