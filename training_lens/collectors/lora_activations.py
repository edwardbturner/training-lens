...
class LoRAActivationsCollector(DataCollector):
        ...
        for name, module in model.named_modules():
            if hasattr(module, "lora_A") and hasattr(module, "lora_B"):
                if (
                    ...
                    and self.adapter_name in module.lora_A
                    and isinstance(module.lora_B, dict)
                    and self.adapter_name in module.lora_B
                ):
                    try:
                        # Hook input to LoRA path (pre-A)
                        pre_hook = module.register_forward_pre_hook(self._make_pre_lora_hook(name))
                        self.hooks.append(pre_hook)

                        # Hook A matrix output (post-A, pre-B)
                        lora_A = module.lora_A[self.adapter_name]
                        a_hook = lora_A.register_forward_hook(self._make_post_a_hook(name))
                        self.hooks.append(a_hook)

                        # Hook B matrix output (post-B)
                        lora_B = module.lora_B[self.adapter_name]
                        b_hook = lora_B.register_forward_hook(self._make_post_b_hook(name))
                        self.hooks.append(b_hook)

                        # Hook main module output for comparison
                        main_hook = module.register_forward_hook(self._make_main_output_hook(name))
    ...
    def _compute_lora_metrics(self, module_activations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        available_keys = [key for key in required_keys if key in module_activations]
        ...
        try:
            ...
            if "pre_lora" in module_activations and "post_b" in module_activations:
                ...
                if len(pre_flat) < max_len:
                    pre_flat = torch.cat([pre_flat, torch.zeros(max_len - len(pre_flat))])
                if len(post_flat) < max_len:
                    post_flat = torch.cat([post_flat, torch.zeros(max_len - len(post_flat))])
