...
class AdapterGradientsCollector(DataCollector):
    ...
    def collect(self, model: torch.nn.Module, step: int, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        for name, module in model.named_modules():
            if hasattr(module, "lora_A") and hasattr(module, "lora_B"):
                ...
                if (
                    isinstance(module.lora_A, dict)
                    and adapter_name in module.lora_A
                    and isinstance(module.lora_B, dict)
                ...
                if "A_grad_norm" in gradients_data and "B_grad_norm" in gradients_data:
                    gradients_data["grad_norm_ratio"] = gradients_data["A_grad_norm"] / (
                ...
                if "A_gradient" in gradients_data and "B_gradient" in gradients_data:
                    ...
                    if len(grad_A_flat) < max_len:
                        grad_A_flat = torch.cat([grad_A_flat, torch.zeros(max_len - len(grad_A_flat))])
                    if len(grad_B_flat) < max_len:
                        grad_B_flat = torch.cat([grad_B_flat, torch.zeros(max_len - len(grad_B_flat))])
    ...
    def _has_lora_adapters_with_gradients(self, model: torch.nn.Module) -> bool:
        """Check if model has LoRA adapters with gradients."""
        # Check for PEFT-style adapters
        for module in model.modules():
            if hasattr(module, "lora_A") and hasattr(module, "lora_B"):
                # Handle dictionary-style (PEFT) adapters
                if isinstance(getattr(module, "lora_A", None), dict):
                    for adapter_name in module.lora_A:
                        lora_A = module.lora_A[adapter_name]
                        lora_B = module.lora_B[adapter_name]
                        if lora_A.weight.grad is not None or lora_B.weight.grad is not None:
                            return True
                # Handle direct Linear layer style adapters
                elif hasattr(module.lora_A, "weight") and hasattr(module.lora_B, "weight"):
                    if module.lora_A.weight.grad is not None or module.lora_B.weight.grad is not None:
                        return True

        # Also check for gradient on any parameter with 'lora' in name
